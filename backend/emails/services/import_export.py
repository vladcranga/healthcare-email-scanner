...
def validate_file_extension(file) -> str:
    ...
    if filename.endswith('.json'):
        ...
        raise ValidationError('Unsupported file type. Only JSON and CSV files are allowed.')
...
def export_emails_to_csv(emails: List[Email]) -> str:
    ...
    # Write header
    writer.writerow(['sender', 'subject', 'content', 'status', 
                    'confidence_score', 'received_date', 'is_quarantined'])

...
from backend.emails.models import Email, EmailAttachment, SuspiciousPattern, EmailAnalysis
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--clear', action='store_true', help='Clear existing emails before generating new ones')
    ...
    def generate_healthcare_content(self, fake, is_suspicious, is_dangerous=False):
        ...
        if is_dangerous:
            ...
            subject = "Discount Prescription Medication" if "prescription" in content else "Private Healthcare Offer"
        else:
            # Safe content
            templates = [
    ...
    def create_suspicious_patterns(self):
        ...
        patterns = [
            {"pattern": r"\b\d{3}\s*\d{3}\s*\d{4}\b", "category": "NHS_NUMBER", "severity": 9, 
             "description": "NHS Number in plain text", "is_regex": True},
            {"pattern": r"\b[A-Z]{2}\s*\d{10,20}\b", "category": "EHIC_NUMBER", "severity": 8,
             "description": "EHIC Number in plain text", "is_regex": True},
            {"pattern": "skip nhs waiting", "category": "SERVICE_SCAM", "severity": 7,
             "description": "Private healthcare scam"},
            {"pattern": "no prescription needed", "category": "MEDICATION_SCAM", "severity": 8,
             "description": "Illegal medication sale"},
            {"pattern": "discount prescription", "category": "MEDICATION_SCAM", "severity": 7,
             "description": "Medication discount scam"},
            {"pattern": "urgent test results", "category": "EMERGENCY_SCAM", "severity": 8,
             "description": "Urgent medical results scam"},
            {"pattern": "ehic renewal", "category": "INSURANCE_SCAM", "severity": 7,
             "description": "EHIC renewal scam"},
            {"pattern": "urgent transfer", "category": "phishing", "severity": 8,
             "description": "Urgent financial transfer request"},
            {"pattern": "account suspended", "category": "phishing", "severity": 7,
             "description": "Warning about account suspension"},
            {"pattern": "click here immediately", "category": "spam", "severity": 9,
             "description": "Urgent call to action"},
            {"pattern": "wire transfer", "category": "fraud", "severity": 6,
             "description": "Suspicious wire transfer request"},
            {"pattern": "confidential information", "category": "data-leak", "severity": 5,
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(dangerous_count):
            subject, content = self.generate_healthcare_content(fake, is_suspicious=True, is_dangerous=True)
            email_data = {
                'sender': fake.email(),
                'subject': subject,
                'content': content,
                ...
                'received_date': (datetime.now() - timedelta(days=random.randint(0, 30))).isoformat(),
                'is_quarantined': True,
            }
        ...
        for _ in range(suspicious_count):
            subject, content = self.generate_healthcare_content(fake, is_suspicious=True)
            email_data = {
                'sender': fake.email(),
                'subject': subject,
                'content': content,
        ...
        for _ in range(safe_count):
            subject, content = self.generate_healthcare_content(fake, is_suspicious=False)
            email_data = {
                'sender': fake.email(),
                'subject': subject,
                'content': content,

...
export const authService = {
  ...
  refreshToken: () => api.post("/api/auth/login/refresh/"),
...
export const emailService = {
  getEmails: (params: { status?: string; search?: string; page?: number }) => {
    return api.get<{
      count: number;
      next: string | null;
  ...
  getSuspiciousEmails: () => api.get("/api/emails/?status=suspicious"),
  ...
  getSuspiciousSummary: () => api.get("/api/emails/suspicious_summary/"),

  getPublicStats: () => api.get("/api/emails/public_stats/"),
  ...
  deleteEmail: (emailId: number) => api.delete(`/api/emails/${emailId}/`),
};

...
from backend.emails.models import Email, SuspiciousPattern, EmailAnalysis, EmailAttachment

print('Suspicious Patterns:')
for pattern in SuspiciousPattern.objects.all().values('pattern', 'category', 'severity'):
    print(pattern)
...
for email in suspicious_emails:
    print(f'Sender: {email.sender}, Subject: {email.subject}, Confidence Score: {email.confidence_score}')
...
if suspicious_emails:
    ...
    for pattern in analysis.matched_patterns.all():
        print(f'- {pattern.pattern} (Category: {pattern.category}, Severity: {pattern.severity})')
